...
class SCEASAR_OT_mod_export_master_fbx(bpy.types.Operator):
    """Export selected collections as FBX with version number"""
    bl_idname = "sceasar.export_master_fbx"
    ...
    def execute(self, context):
        ...
        export_dir = os.path.join(blend_dir, "master", blend_name)
...
def menu_func(self, context):
    self.layout.operator(SCEASAR_OT_mod_export_master_fbx.bl_idname)


def register():
    bpy.utils.register_class(SCEASAR_OT_mod_export_master_fbx)
    bpy.types.TOPBAR_MT_file_export.append(menu_func)


def unregister():
    bpy.utils.unregister_class(SCEASAR_OT_mod_export_master_fbx)
